...
async function fetchImageSize(
  ...
  try {
    ...
    console.log(
      `FetchImageSize error. repo: ${from} , image: ${URL} , error: ${err}`,
    );
    return null;
  }
}
...
async function httpGet(URL: string): Promise<any> {
  ...
  return (await axios.default.get(URL, {
    headers: {
      "Authorization":
        "Basic dXNlcm5hbWU6Z2hwX21qd2hFeFNNaFVTTEpPMHJsNVQycXM4TzJMTkVacDNoMDBubA==",
    },
  }));
}

async function repoAPI(user: string, repoName: string): Promise<any> {
  let URL = `https://api.github.com/repos/${user}/${repoName}`;
  let response = await httpGet(URL);
  console.log(
    `[${response.headers["x-ratelimit-remaining"]
    }]Got Repo: https://github.com/${user}/${repoName}`,
  );
  return response.data;
}

async function repoReadmeAPI(user: string, repoName: string): Promise<any> {
  let URL = `https://api.github.com/repos/${user}/${repoName}/readme`;
  let response = await httpGet(URL);
  console.log(
    `[${response.headers["x-ratelimit-remaining"]}]Got Readme: ${repoName}`,
  );
  return response.data;
}
...
async function fetchRepo(URL: string): Promise<Repo> {
  ...
  const markdownImageRegExp = RegExp(
  ...
  imageURLs = imageURLs.map((imageURL) => {
    if (!imageURL.startsWith("http")) {
      ...
      imageURL =
        `https://github.com/${user}/${repoName}/raw/${branchName}${imageURL.startsWith("/") ? "" : "/"
        }` + imageURL;
    }
  ...
  // size sort
  var imagesURLAndSize = await Promise.all<[string, [number, number] | null]>(
    imageURLs.map<Promise<[string, [number, number] | null]>>(
      (imageURL) =>
        fetchImageSize(imageURL, URL).then((size) => [imageURL, size]),
    ),
  );
  var nonNullImagesURLAndSize = imagesURLAndSize.flatMap<[string, [number, number]]>((e) => e[1] != null ? [e as [string, [number, number]]] : []);

  nonNullImagesURLAndSize.sort((a, b) => {
...
hexo.extend.renderer.register("github", "html", async function (data, options) {
  try {
    ...
    let htmlPromise: Promise<string> = (hexo.extend.renderer as any).get("md")
      .call(hexo, {
        text: markdown,
        path: data.path,
      }, options);
...
function scheduleRequests() {
  let lastInvocationTime: number | null;
